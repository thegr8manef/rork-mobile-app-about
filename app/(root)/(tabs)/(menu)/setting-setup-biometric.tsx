...
import { signChallenge } from "@/native/SecureSignModule";
...
export default function SetupBiometricScreen() {
  ...
  const handleEnableBiometric = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Device should already be trusted from passkey/PIN setup
      // If not trusted, something went wrong in the flow
      if (!authState.hasTrustedDevice) {
        console.log("❌ Device not trusted — redirect to device confidence");
        showMessageError(
          t("common.error"),
          t("biometric.setup.requirePasskey"),
        );
        // Redirect back to device confidence screen
        router.replace("/(auth)/device-confidence");
        return;
      }

      // ✅ Create challenge for biometric verification
  ...
  const getIcon = () => {
    if (biometricLabel === "Face ID") {
      return (
        <ScanFace size={IconSize.xxl * 1.67} color={BankingColors.primary} />
      );
    }
    return (
      <Fingerprint size={IconSize.xxl * 1.67} color={BankingColors.primary} />
    );
  };
